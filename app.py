# app.py
"""
This code version is - 1.11 every time if you make any small update increase this number for tracking purposes - last update 1.10
...
# -------------------------
# S10 Trailing Stop Helpers (integrated – no separate module)
# -------------------------
# These helpers implement the Plan A S10 trailing logic in a pure, side-effect-free way.
# Caller is responsible for fetching ATR/ADX/swing levels and placing orders.
...
class S10TrailInputs:
    ...
    time_since_entry_bars: int        # on execution TF (e.g., M15)
    be_applied: bool                  # whether BE move already done
    trailing_activated: bool          # allow trailing or still gated
    is_stacked_trade: bool            # AA + VBM stacked
    tick_size_abs: float              # absolute tick size
    taker_fee_pct: float              # e.g., 0.0007
    adx_period: int = 14              # informational

@dataclass
class S10TrailOutputs:
    be_stop: Optional[float]
    new_trailing_sl: Optional[float]
    partial_close_pct: Optional[float]
    activate_trailing: bool
    reason: str

def _s10_get(cfg: dict, key: str, default):
    v = cfg.get(key, default)
    if isinstance(v, str) and v.lower() in ("true", "false"):
        return v.lower() == "true"
    return v

def s10_load_trailing_config(path: Optional[str]) -> dict:
    # Defaults
    cfg: dict = {
...
def _s10_r_multiple(side: str, entry: float, price: float, initial_risk_per_unit: float) -> float:
...
def _s10_min_sl_move_threshold(cfg: dict, current_price: float, tick_size_abs: float) -> float:
...
def _s10_round_to_tick(price: float, tick: float, mode: str) -> float:
    if tick <= 0:
        return price
    if mode == "down":
...
def s10_compute_trailing_update(cfg: dict, inputs: S10TrailInputs) -> S10TrailOutputs:
    ...
    r_mult = _s10_r_multiple(side, inputs.entry_price, inputs.current_price, r_per_unit)
    be_trigger_r = cfg.get("S10_BE_TRIGGER_R", 1.0)
    be_trigger_pct = cfg.get("S10_BE_TRIGGER_PCT")

    # Gate: R-based or pct-based
    passed_gate = False
    ...
    if side == "BUY":
        ...
        new_sl = _s10_round_to_tick(new_sl, inputs.tick_size_abs, mode="down")
    else:
        ...
        new_sl = _s10_round_to_tick(new_sl, inputs.tick_size_abs, mode="up")

    min_move = _s10_min_sl_move_threshold(cfg, price, inputs.tick_size_abs)
